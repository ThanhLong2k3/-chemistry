...
export const PermissionTree = ({ roleId, onSelectionChange }: Props) => {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      if (allPermsResponse.success) {
        ...
        throw new Error(
          allPermsResponse.message || 'Lỗi khi tải danh sách quyền.'
        );
      }
      ...
      if (rolePermsResponse.success) {
        ...
        throw new Error(
          rolePermsResponse.message || 'Lỗi khi tải quyền của vai trò.'
        );
      }
    } catch (err: any) {
  ...
  if (treeData.length === 0)
    return <Empty description="Không có dữ liệu phân quyền" />;
