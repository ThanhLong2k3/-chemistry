...
export const SubjectDelete = ({ id, deleted_by, getAllSubject }: Props) => {
  const { isOpen, open, close } = useDisclosure();
  const { show } = useNotification();

  const handleSubmit = async () => {
    try {
      const data = await deleteSubject({ id, deleted_by });

      if (data.success) {
        show({
          result: 0, // Mã thành công
          messageDone: 'Xóa môn học thành công!',
        });
        getAllSubject(); // Tải lại dữ liệu
      } else {
